//@ sourceURL=basicdatasetTree.js
// ztree的参数以及相关方法
//zTreeId 生成树的ul标签的ID；data 用于生成树的数据
function InitbasicdatasetzTree(zTreeId,data){
    var basicdataset_treeObj = $.fn.zTree.init($("#"+zTreeId), basicdatasetsetting,data);//初始化treeResouce用作增加和编辑的选择框
    basicdataset_treeObj.expandAll(true);//节点全部展开
}
var basicdatasetsetting = {
    view: {
    ...
    callback: {
        ...
        onClick: onClickbasicdataset//回调函数
    }
};
//点击事件，通过点击来获取树的节点的字段和id赋值给form表单的相应字段
function onClickbasicdataset(e, treeId, treeNode) {
    var idKey = basicdatasetsetting.data.simpleData.idKey;
    var name = basicdatasetsetting.data.key.name;
    var pIdKey = basicdatasetsetting.data.simpleData.pIdKey;
    ...
    if(formId.toLowerCase().indexOf("employee") >= 0 || formId.toLowerCase().indexOf("serialnumber") >= 0  ){
        ...
        $("#"+formId+" input[name='CnName']").val(v);//employee中的组织架构树
    }
    // else if(){
...
function showbasicdatasetMenu(eachMenuContent) {
