...
//					alert(JSON.stringify($('#searchForm').serializeObject()));
                ...
                columns: [
                    ...
                    {title: "日志编号", data: "logId"},
                    {title: "用户编号", data: "logUserId"},
                    {title: "操作内容", data: "logClass"},
                    {title: "操作类型", data: "logFunction"},
                    // {title: "操作内容", data: "logMessage"},
                    {title: "操作时间", data: "logTime"}
                ],
                columnDefs: [
                    {
                        orderable: false,
    ...
    $('body').on('click', '.container-type .checkall', function () {
        ...
        $(".log .checkchild").prop("checked", check);
        //通过调用datatables的select事件来触发选中
        $("#log_table tbody tr").each(function () {
            if ( log_table.row( this, { selected: true } ).any() ) {
                log_table.row( this ).deselect();
            }
            else {
                log_table.row( this ).select();
            }
        });

    });
    $('#log_table tbody').on('click', 'tr', function () {
...
//     var data = log_table.rows($(this)).data()[0];
//     $("#detail_table").html("");
