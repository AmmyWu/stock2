...
var Administration = (function() {
    ...
    $.validator.setDefaults({
        submitHandler: submitEditAdministrationModal
    });
    ...
    $().ready(
        function validateCommonsetForm() {
            Commonset_Validator = $("#editAdministrationForm").validate({
                rules: {
                	 type: {
                         maxlength: 30,
                         digits: true
    ...
    var paral = {
    		 "name": "名称",
             //"governmentId":"地区ID",
             "organizationStructureId": "组织架构id", // organizationStructureId + creator
    ...
    function InitTable() {
        ...
        $.ajax({
            ...
            "data": {
                ...
                /*keys: JSON.parse($.cookie('loginingEmployee'))['customer']['customerId']//JSON.stringify($('#searchAdministrationForm').serializeObject())
*/            },
            // async:false,
            dataType: "json",
            async: false,
            success: function (predata) {
                var data = predata;
                maxcode = data[data.length-1].organizationStructureId;
                if(data.length!=0){
                	$("#Administration_import-excel").attr("disabled","true");
                }
                Administration_table = data;//为子对象展开使用
                //初始化form弹框的资源树
                InitAdministrationzTree("administrationTreeResouce", data);
    ...
    function addAdministration() {
        ...
        setDefaultValue($("#editAdministrationModalBody"), 'insert');

        $('#editAdministrationModal').modal('show');//现实模态框

        // });
    }

    function submitEditAdministrationModal() {
        //$("#editDutiesModalSubmit").click(function () {
       
        var saveType = $("#editAdministrationForm input[name='saveType']").val();

        // insert 才增加，update 用原来的organizationStructureId
        if (saveType == 'insert') {
            $("#editAdministrationForm input[name='organizationStructureId']").val(maxcode+1);
        }
        var data = $('#editAdministrationForm').serializeObject();
        $.ajax({
            type: 'POST',
        ...
        $('#editAdministrationModal').modal('hide');//现实模态
    }


    /*
     * 根据organizationStructureId和creator来修改删除地区
     */
    function deleteAdministration() {
        ...
        callAlertModal(info,'Administration_confirmDelete');

        //是否包含父节点
        var isContainParent = false;

        $('.Administration_confirmDelete').unbind('click').click(function () {
            var reqData;
            var parentId;
            $("#resourceTreetable tr").each(function () {
                if ($(this).hasClass('selected')) {
                    ...
                    reqData = {
                        organizationStructureId: $(this).find("td[title='organizationStructureId']")[0].innerHTML,
                ...
                success: function (res) {
                    ...
                    if (res.code == 0) {
                        ...
                        if (parentId != "undefined" || parentId != undefined) {
                            $("#resourceTreetable").treetable("reveal", parentId);
                            // // 编辑或者增加保存成功后展开该节点的父节点
                        }
                    } else {
    ...
    function editAdministration() {
        ...
        $("#resourceTreetable tr").each(function () {
            ...
            if ($(this).hasClass('selected')) {
                ...
                for (var key in obj) {
                    $("#editAdministrationModal input[name='" + key + "']").val(obj[key]);
                }


                $("#editAdministrationForm input[name='amendTime']").val(  obj['amendTime']);

                setDefaultValue($("#editAdministrationModalBody"), 'update');
    ...
    function importExcel() {
        $('#form1Administration input[name=excel]').val("");
        $('#importExcelAdministrationModal').modal('show');//现实模态框
    }

    function importExcel2(formId, selectedFile, showUploadedFileLabelId) {
        // var modelName = "";
        var titleName = $('#form1Administration input[name=excel]').val();
        if (!titleName) {
            callAlert("请选择文件");
            return;
        }
        ...
        $.ajax({
            ...
            url: getContextPath() +"/organization/importExcel.do",
            data: formdata,
            contentType: false,
            cache: false,
            processData: false,
    ...
    function emptyAddForm() {
        $("#editAdministrationForm")[0].reset();
...
// click item display detail infomation
    ...
    $('#showAdministrationDetail').on('click', function () {
